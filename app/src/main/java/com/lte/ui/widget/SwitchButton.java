...
import com.lte.R;
import com.lte.utils.ColorUtils;
...
public class SwitchButton extends CompoundButton {
	...
	public void setSelectBackGroundDrawableWithColor(@ColorInt int selectColor, @ColorInt int selectStrokeColor, @DrawableRes int selectRes, @ColorInt int unSelectColor, @ColorInt int unSelectStrokeColor, @DrawableRes int unSelectRes){
		GradientDrawable selected = null;
		GradientDrawable unSelected = null;
		try{
			selected = (GradientDrawable) getContext().getResources().getDrawable(selectRes);
			unSelected = (GradientDrawable) getContext().getResources().getDrawable(unSelectRes);
		}catch (Exception e){
			e.printStackTrace();
			return;
		}
		selected.setColor(selectColor);
		selected.setStroke(1,selectStrokeColor);

		unSelected.setColor(unSelectColor);
		unSelected.setStroke(1,unSelectStrokeColor);
