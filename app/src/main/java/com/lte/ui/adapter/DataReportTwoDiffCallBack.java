...
public class DataReportTwoDiffCallBack extends DiffUtil.Callback {


    private List<MacData> mOldDatas;
    private List<MacData> mNewDatas;

    public DataReportTwoDiffCallBack(List<MacData> mOldDatas, List<MacData> mNewDatas) {
    ...
    public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
        MacData beanOld = mOldDatas.get(oldItemPosition);
        MacData beanNew = mNewDatas.get(newItemPosition);
    ...
    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
        MacData beanOld = mOldDatas.get(oldItemPosition);
        MacData beanNew = mNewDatas.get(newItemPosition);
        return TextUtils.equals(beanNew.getSerialNumber(),beanOld.getSerialNumber())
                && beanOld.getMac() == beanNew.getMac() && beanNew.getTime() == beanOld.getTime();
    }
    ...
    public Object getChangePayload(int oldItemPosition, int newItemPosition) {
        ...
        if (!TextUtils.equals(beanOld.getSerialNumber(), beanNew.getSerialNumber())) {
            bundle.putString(Constants.SERIALNUMBER, beanNew.getSerialNumber());
        }
        if (!TextUtils.equals(beanOld.getMac(), beanNew.getMac())) {
            bundle.putString(Constants.MAC, beanNew.getMac());
        }
